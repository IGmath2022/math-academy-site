      ...
      {progress.length === 0
        ...
        : (
          <ul style={{ fontSize: 15, padding: 0, margin: 0 }}>
            {progress.map(p => (
              <li key={p.id} style={{ marginBottom: 7, lineHeight: 1.6 }}>
...
function StudentProgressCalendar({ userId, chapters }) {
  ...
  function dateToYMD(date) {
    ...
    const ymd = new Date(date.getTime() - tzOffset).toISOString().slice(0, 10);
    return ymd;
  }
        ...
        <div style={{ marginTop: 12, fontSize: 14, background: "#f8fafb", borderRadius: 7, padding: 9 }}>
          ...
          <ul style={{ margin: 6, padding: 0 }}>
            {progressMap[selectedDate].map(p => (
              <li key={p.id}>
                <b>{chapters.find(c => c.id === p.chapterId)?.name || p.chapterId}</b>
    ...
    <div style={{
      ...
      <div style={{
        ...
        <button onClick={onClose}
          style={{ position: "absolute", top: 13, right: 14, border: "none", background: "none", fontSize: 23, fontWeight: 800, color: "#456", cursor: "pointer" }}>×</button>
        ...
        <div style={{ marginBottom: 12 }}>
          <button onClick={() => setShowCalendar(false)} style={{ marginRight: 7, background: showCalendar ? "#e4e9f2" : "#226ad6", color: showCalendar ? "#333" : "#fff", border: "none", borderRadius: 7, padding: "6px 12px" }}>
            상세 정보
          </button>
          <button onClick={() => setShowCalendar(true)} style={{ background: showCalendar ? "#226ad6" : "#e4e9f2", color: showCalendar ? "#fff" : "#333", border: "none", borderRadius: 7, padding: "6px 12px" }}>
            진도 달력
          </button>
        </div>
        {!showCalendar ? (
          <>
            ...
            <div style={{ marginTop: 18, textAlign: "right" }}>
              ...
              {edit && <button onClick={() => setEdit(false)} style={{ padding: "7px 12px", borderRadius: 7, background: "#e3e5ec", border: "none" }}>취소</button>}
              {!edit && (
                <>
            ...
            <StudentProgressHistory userId={student.id} chapters={chapters} />
          </>
        ) : (
          <StudentProgressCalendar userId={student.id} chapters={chapters} />
        )}
      </div>
    </div>
  );
